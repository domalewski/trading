...
function killSynth(cb) {
	...
	try {
		...
		mGain.gain.linearRampToValueAtTime(0, actx.currentTime + 1.4);
	} catch (e) {}
	setTimeout(() => {
		anodes.forEach((n) => {
			try {
...
function mkCtx() {
	...
	mGain.gain.linearRampToValueAtTime(0.5, actx.currentTime + 3);
	mGain.connect(actx.destination);
}
function pinkBuf(sec) {
	const len = actx.sampleRate * sec,
		buf = actx.createBuffer(2, len, actx.sampleRate);
...
function loopNoise(freq, type, vol) {
	const src = actx.createBufferSource();
	src.buffer = pinkBuf(4);
	src.loop = true;
	const f = actx.createBiquadFilter();
	f.type = type || "lowpass";
	f.frequency.value = freq || 600;
...
function pad(freqs, type, vol, lfo) {
	freqs.forEach((f, i) => {
...
function drops() {
	const go = () => {
		...
		const o = actx.createOscillator(),
			g = actx.createGain();
		o.frequency.value =
			[523, 659, 784, 1047][Math.floor(Math.random() * 4)] *
			(1 + Math.random() * 0.015);
		o.type = "sine";
		const n = actx.currentTime;
		g.gain.setValueAtTime(0, n);
		g.gain.linearRampToValueAtTime(0.045, n + 0.012);
		g.gain.exponentialRampToValueAtTime(0.0001, n + 1.3);
		o.connect(g);
		g.connect(mGain);
		o.start(n);
		o.stop(n + 1.4);
		setTimeout(go, 1600 + Math.random() * 3800);
	};
	setTimeout(go, 700);
}
function bells() {
	const go = () => {
		...
		[1, 2.756, 5.4, 8.93].forEach((p) => {
			...
			const n = actx.currentTime;
			g.gain.setValueAtTime(0, n);
			g.gain.linearRampToValueAtTime(0.036 / (p * 0.5 + 0.5), n + 0.02);
			g.gain.exponentialRampToValueAtTime(0.0001, n + 5 + Math.random() * 4);
			o.connect(g);
			g.connect(mGain);
			o.start(n);
			o.stop(n + 10);
		});
		setTimeout(go, 8000 + Math.random() * 13000);
	};
	setTimeout(go, 1500);
}
function crickets() {
	const go = () => {
		...
		g.connect(mGain);
		s.start();
		setTimeout(go, 55 + Math.random() * 110);
	};
	setTimeout(go, 2200);
}
function owl() {
	const go = () => {
		...
		const dur = 0.45 + Math.random() * 0.35,
			...
			g = actx.createGain();
		o.type = "sine";
		o.frequency.setValueAtTime(315, actx.currentTime);
		o.frequency.linearRampToValueAtTime(275, actx.currentTime + dur);
		g.gain.setValueAtTime(0, actx.currentTime);
		g.gain.linearRampToValueAtTime(0.045, actx.currentTime + 0.1);
		g.gain.setValueAtTime(0.045, actx.currentTime + dur - 0.1);
		g.gain.linearRampToValueAtTime(0, actx.currentTime + dur);
		o.connect(g);
		g.connect(mGain);
		o.start();
		o.stop(actx.currentTime + dur + 0.05);
		setTimeout(go, 9000 + Math.random() * 17000);
	};
	setTimeout(go, 6000);
}

/* Nature: rain + wind + water sounds */
function rainSound() {
	loopNoise(1200, "lowpass", 0.38);
	loopNoise(400, "lowpass", 0.28);
	loopNoise(5500, "bandpass", 0.06);
}
function windSound() {
	// Layered wind: low rumble + mid whoosh + high whistle
	loopNoise(180, "lowpass", 0.18);
	loopNoise(620, "bandpass", 0.12);
	// LFO-modulated wind oscillation
	const o = actx.createOscillator(),
...
function waterDrops() {
	// Gentle water drop sounds for lake
	const go = () => {
		if (!actx || !mGain) return;
		const o = actx.createOscillator(),
			g = actx.createGain();
		...
		o.frequency.setValueAtTime(baseFreq * 1.5, actx.currentTime);
		...
		const n = actx.currentTime;
		g.gain.setValueAtTime(0, n);
		g.gain.linearRampToValueAtTime(0.028, n + 0.008);
		g.gain.exponentialRampToValueAtTime(0.0001, n + 0.4);
		o.connect(g);
		g.connect(mGain);
		o.start(n);
		o.stop(n + 0.5);
		setTimeout(go, 800 + Math.random() * 3500);
	};
	setTimeout(go, 1200);
}

function whoosh(isIn) {
	if (!actx || !mGain) return;
	const dur = isIn ? 3.6 : 4.0,
		buf = actx.createBuffer(1, actx.sampleRate * dur, actx.sampleRate),
		d = buf.getChannelData(0);
	for (let i = 0; i < d.length; i++) {
		const tv = i / d.length;
		d[i] =
			(Math.random() * 2 - 1) *
			(isIn ? Math.sin(tv * Math.PI) : Math.sin((1 - tv) * Math.PI) * 0.82) *
			0.15;
	}
	const s = actx.createBufferSource();
	s.buffer = buf;
	const bp = actx.createBiquadFilter();
	bp.type = "bandpass";
	bp.frequency.value = isIn ? 2100 : 1650;
	bp.Q.value = 0.5;
	const g = actx.createGain();
	g.gain.value = 0.62;
	s.connect(bp);
...
function synthJazz() {
	const bassNotes = [87.31, 98, 110, 130.81, 146.83, 130.81, 110, 98];
	let beatIdx = 0;
	const tempo = 1.4;
	const bassWalk = () => {
		if (!actx || !mGain) return;
		const o = actx.createOscillator(),
			g = actx.createGain();
		o.type = "triangle";
		o.frequency.value = bassNotes[beatIdx % bassNotes.length];
		const n = actx.currentTime;
		g.gain.setValueAtTime(0, n);
		g.gain.linearRampToValueAtTime(0.18, n + 0.04);
		g.gain.exponentialRampToValueAtTime(0.0001, n + tempo * 0.85);
		o.connect(g);
		g.connect(mGain);
		o.start(n);
		o.stop(n + tempo);
		beatIdx++;
		setTimeout(bassWalk, tempo * 1000);
	};
	bassWalk();
	const chordSets = [
		[174.61, 220, 293.66],
		[195.99, 246.94, 329.63],
		[220, 277.18, 369.99],
		[174.61, 220, 293.66]
	];
	let chordIdx = 0;
	const strumChord = () => {
		...
		chord.forEach((freq, i) => {
			...
			filt.Q.value = 1.2;
			const n = actx.currentTime + i * 0.012;
			g.gain.setValueAtTime(0, n);
			g.gain.linearRampToValueAtTime(0.055, n + 0.08);
			g.gain.exponentialRampToValueAtTime(0.0001, n + tempo * 3.8);
			o.connect(filt);
			filt.connect(g);
			g.connect(mGain);
			o.start(n);
		...
		chordIdx++;
		setTimeout(strumChord, tempo * 4 * 1000);
	};
	...
	const swingTick = () => {
		...
		setTimeout(swingTick, tempo * 1000);
	};
	setTimeout(swingTick, 100);
	const riffNotes = [349.23, 392, 440, 493.88, 523.25, 440, 392, 349.23];
	const playRiff = () => {
		if (!actx || !mGain) return;
		riffNotes.forEach((freq, i) => {
			...
			filt.Q.value = 2;
			const n = actx.currentTime + i * tempo * 0.5;
			g.gain.setValueAtTime(0, n);
			g.gain.linearRampToValueAtTime(0.07, n + 0.05);
			g.gain.exponentialRampToValueAtTime(0.0001, n + tempo * 0.45);
			o.connect(filt);
			filt.connect(g);
			g.connect(mGain);
			o.start(n);
		...
		setTimeout(playRiff, 14000 + Math.random() * 8000);
	};
	setTimeout(playRiff, 2000);
}

function playMusicTrack(tracks, synthFallback) {
	const shuffled = [...tracks].sort(() => Math.random() - 0.5);
	const tn = document.getElementById("track-name");
	let tried = 0;
	const tryNext = () => {
		if (tried >= shuffled.length) {
			tn.textContent = "Live Synthesis";
			tn.style.color = T[mode].cueCol;
			tn.classList.add("show");
			if (synthFallback) synthFallback();
			return;
		}
		const track = shuffled[tried++];
		...
		musicAudio.addEventListener(
			...
			() => {
				...
				const fade = setInterval(() => {
					if (!musicAudio) return clearInterval(fade);
					vol = Math.min(vol + 0.015, 0.46);
					musicAudio.volume = vol;
					if (vol >= 0.46) clearInterval(fade);
				}, 120);
			},
			{ once: true }
		);
...
function startAudio() {
	...
	const tn = document.getElementById("track-name");
	if (mode === "classical") {
		loopNoise(220, "lowpass", 0.1);
		pad([110, 146.83, 220], "sine", 0.036, 0.04);
		playMusicTrack(CLASSICAL_TRACKS, () => {
			pad([130.81, 164.81, 196, 261.63], "sine", 0.045, 0.05);
			bells();
		});
	} else if (mode === "jazz") {
		loopNoise(300, "lowpass", 0.07);
		playMusicTrack(JAZZ_TRACKS, synthJazz);
	} else if (mode === "nature") {
		tn.classList.remove("show");
		rainSound();
		windSound();
		waterDrops();
		pad([82.41, 110, 130.81], "sine", 0.022, 0.04);
	} else if (mode === "breathe") {
		tn.classList.remove("show");
		const pool = [
			() => {
				loopNoise(650, "lowpass", 0.4);
				loopNoise(180, "highpass", 0.09);
				pad([65.41, 98, 130.81], "sine", 0.07, 0.08);
				drops();
			},
			() => {
				loopNoise(480, "lowpass", 0.44);
				pad([55, 82.41, 110], "sine", 0.074, 0.06);
				bells();
			}
		];
		...
		const pool = [
			() => {
				loopNoise(380, "lowpass", 0.25);
				pad([110, 146.83, 164.81, 220], "sine", 0.056, 0.05);
				bells();
			},
			() => {
				loopNoise(300, "lowpass", 0.21);
				pad([82.41, 110, 164.81, 246.94], "sine", 0.062, 0.04);
				drops();
			}
		];
		pool[Math.floor(Math.random() * pool.length)]();
		...
		const pool = [
			() => {
				loopNoise(580, "lowpass", 0.29);
				loopNoise(140, "highpass", 0.08);
				pad([55, 73.42, 87.31], "sine", 0.06, 0.07);
				crickets();
				owl();
			},
			() => {
				loopNoise(750, "lowpass", 0.4);
				loopNoise(3500, "bandpass", 0.036);
				pad([65.41, 87.31, 130.81], "sine", 0.055, 0.06);
				crickets();
				drops();
			}
		];
